...
angular.module('HotelReview')
  .controller('DetailController', ['APP_CONFIG', 'GPlace', '$localStorage', '$ionicPopup', '$cordovaSocialSharing', '$state', '$log', '_', 
    ...
    $ionicPopover.fromTemplateUrl('reviewsummary-popover.html', {
      ...
      $log.debug('Review summary popover has opened');
      $log.debug(popover);
    ...
    this.showFullscreenImage = function(index){
      $log.debug('Show fullscreen image ' + index);
      if(!index) {
        index = 0;
      }
    ...
    $scope.$on('$destroy', function() {
      ctrl.modal.remove();
      ctrl.reviewsummaryPopover.remove();
    });
  }]);